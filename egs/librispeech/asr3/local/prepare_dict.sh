#!/usr/bin/env bash
# Prepares two phone-based dictionaries for LibriSpeech from the official lexicon,
...
[ ! -f $lex ] && echo "$0: lexicon file not found at $lex" && exit 1;
...
# get phone-level dictionary
awk 'FNR==NR {words[$1]; next} $1 in words' $tmpdir/words.eval $lex > $tmpdir/raw.lexicon.txt
awk '{ $1=$1; print }' FS='[ \t]+' OFS=' ' $tmpdir/raw.lexicon.txt | awk '{print $0, "<eow>"}' | sort | uniq >> $dst_eval_dir/lexicon.txt
cat $dst_eval_dir/lexicon.txt | cut -f 2- -d' '| tr ' ' '\n' | sort | uniq | grep -v -w "<SIL>" | grep -v -w "<UNK>" | awk 'NF>0' >> $dst_eval_dir/nonsilence_phones.txt
...
awk 'FNR==NR {words[$1]; next} $1 in words' $tmpdir/words.train_dev $lex > $tmpdir/raw.lexicon.train_dev.txt
awk '{ $1=$1; print }' FS='[ \t]+' OFS=' ' $tmpdir/raw.lexicon.train_dev.txt | awk '{print $0, "<eow>"}' | sort | uniq >> $dst_dir/lexicon.txt
