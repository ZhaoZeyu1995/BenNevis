...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    utils/prepare_lang.sh --position-dependent-phones false --sil_prob 0.0 data/local/dict "sil" data/local/lang data/lang
    ...
    for topo in ${topos}; do
        if [ $topo == "ctc" ]; then
            torchrun --standalone --nproc_per_node=${ngpu} \
                ...
                logger.name=wav2vec2-${topo} \
                hydra.run.dir=exp/wav2vec2-$topo || exit 1;
        else
            torchrun --standalone --nproc_per_node=${ngpu} \
                run/train.py \
                model=wav2vec2.base \
    ...
    for topo in ${topos}; do
        for x in ${recog_sets}; do
            ./run/predict.sh --ngpu ${ngpu} \
                ...
                exp/wav2vec2-${topo}/checkpoints/best.pt exp/wav2vec2-${topo}/pred_${x} || exit 1;
        done
    done
fi
    ...
    for topo in ${topos}; do
        for x in ${recog_sets}; do
            ./run/decode_faster.sh --nj ${nj} \
                data/${x} data/lang_test_nolm_${topo} \
                exp/wav2vec2-${topo}/pred_${x} exp/wav2vec2-${topo}/dec_nolm_${x} || exit 1;
        done
    done
fi
...
if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
    for topo in ${topos}; do
        for x in ${recog_sets}; do
            run/align.sh data/${x} data/lang_test_nolm_${topo} exp/wav2vec2-${topo}/pred_${x} || exit 1;
        done
    done
fi
...
if [ ${stage} -le 8 ] && [ ${stop_stage} -ge 8 ]; then
    for topo in ${topos}; do
        for x in ${recog_sets}; do
            run/align.sh data/${x} data/lang_test_nolm_${topo} exp/wav2vec2-${topo}/pred_${x} exp/wav2vec2-${topo}/dec_nolm_${x}/aw_1.0-ma_5000-bm_32 || exit 1;
        done
    done
fi
